...
class SystemConfig:
    ...
    VETO_KEYWORDS = [
        ...
        "Federal Funds Rate Decision", "Fed Rate Decision",
...
class RegimeEngine:
    ...
    def generate_mandate(self, score: RegimeScore, vol_metrics: VolMetrics,
        ...
        # ── Bias: requires 3-signal confluence to tilt directional ──────────────
        # Signal 1: PCR ATM (market positioning)
        # Signal 2: FII sentiment (institutional flow)
        # Signal 3: Spot vs MA20 (price momentum) + VIX momentum (fear direction)
        # All must agree before bias tilts from NEUTRAL.
        # This prevents directional spreads based on a single weak signal.
        bias = "NEUTRAL"

        spot = vol_metrics.spot
        ma20 = vol_metrics.ma20
        vix_momentum = vol_metrics.vix_momentum
        fii_sentiment = external_metrics.fii_sentiment
        pcr_atm = struct_metrics.pcr_atm

        price_bullish = spot > ma20
        price_bearish = spot < ma20

        bullish_signals = (
            pcr_atm > 1.3 and
            fii_sentiment == "BULLISH" and
            price_bullish and
            vix_momentum == "FALLING"
        )
        bearish_signals = (
            pcr_atm < 0.7 and
            fii_sentiment == "BEARISH" and
            price_bearish and
            vix_momentum == "FALLING"
        )
        ...
        if bullish_signals:
            ...
            bias = "BEARISH"
        elif pcr_atm > 1.3 and fii_sentiment == "BULLISH":
            bias = "MILDLY_BULLISH"
        elif pcr_atm < 0.7 and fii_sentiment == "BEARISH":
            bias = "MILDLY_BEARISH"
        ...
        if score.total_score >= 7.0:
            suggested_structure = "PROTECTED_STRADDLE"
            wing_protection = "ATM straddle with far OTM wings at 0.02 delta — max premium capture"
            regime = "AGGRESSIVE_SHORT"
        elif score.total_score >= 6.0:
            suggested_structure = "IRON_FLY"
            wing_protection = "ATM short straddle + OTM wings at straddle premium distance"
            regime = "AGGRESSIVE_SHORT"
        elif score.total_score >= 4.0:
            if bias == "NEUTRAL":
                suggested_structure = "IRON_CONDOR"
                wing_protection = "Sell 0.20 delta, buy 0.05 delta wings each side"
            elif "BULL" in bias:
                suggested_structure = "BULL_PUT_SPREAD"
                wing_protection = "Sell 0.30 delta put, buy 0.10 delta put hedge"
            elif "BEAR" in bias:
                suggested_structure = "BEAR_CALL_SPREAD"
                wing_protection = "Sell 0.30 delta call, buy 0.10 delta call hedge"
            else:
                suggested_structure = "IRON_CONDOR"
                wing_protection = "Sell 0.20 delta, buy 0.05 delta wings each side"
            regime = "DEFENSIVE"
...
class StrategyFactory:
    ...
    def construct_iron_fly(self, expiry_date: date, allocation: float, expiry_type: ExpiryType = ExpiryType.WEEKLY) -> Optional[ConstructedStrategy]:
        try:
            ...
            wing_distance = straddle_premium
        ...
        try:
            ...
            professional_recommendation = {
                "primary": {
                    ...
                    "capital_deploy_pct": f"{DynamicConfig.get('WEEKLY_ALLOCATION_PCT') if primary_mandate and primary_mandate.expiry_type == 'WEEKLY' else DynamicConfig.get('MONTHLY_ALLOCATION_PCT') if primary_mandate and primary_mandate.expiry_type == 'MONTHLY' else DynamicConfig.get('NEXT_WEEKLY_ALLOCATION_PCT') if primary_mandate else 0:.0f}% of capital"
                }
            }
